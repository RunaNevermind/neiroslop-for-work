...
// Используем только ОДИН экземпляр для всего
...
async function loadSVGWithDifferentGradients() {
    try {
        addLog('Загрузка SVG с градиентами...');

        // Создаем тестовый SVG если файл не существует
        const testSvg = `./shape.svg`;

        const result = await svgManager.loadSVGWithGradients(testSvg, '#gradientSvgContainer', {
            fillGradient: {
                type: 'linear',
                angle: 45,
                stops: [
        ...
        addLog('✓ SVG с градиентами успешно загружен');
        return result;
    } catch (error) {
        addLog(`✗ Ошибка загрузки SVG с градиентами: ${error.message}`, true);
        // Создаем демо-контент если файл не найден
...
async function loadSVGWithMask() {
    try {
        ...
        const result = await svgManager.loadSVGWithGradients('./shape.svg', '#maskedSvgContainer', {
            maskImage: './mask.png',
            fillGradient: {
                type: 'radial',
                stops: [
